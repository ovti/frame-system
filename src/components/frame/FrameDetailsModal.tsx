      ...
      <div className="space-y-6 sm:space-y-8">
        <div>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
            <h3 className="text-xl font-bold wrap-break-word text-slate-900 sm:text-2xl">
              {frame.name}
            </h3>
          ...
          <p className="mt-2 text-sm leading-6 wrap-break-word text-slate-600 sm:text-base">
            ...
            {frame.parentIds.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {frame.parentIds.map((parentId) => (
                  <span
                    key={parentId}
                    className="max-w-full rounded-full bg-slate-100 px-3 py-1 text-sm wrap-break-word text-slate-700"
                  >
              ...
              frame.slots.map((slot) => (
                ...
                >
                  <div className="mb-3 flex flex-col gap-2 sm:mb-4 sm:flex-row sm:items-center sm:justify-between">
                    <h5 className="text-base font-semibold wrap-break-word text-slate-900 sm:text-lg">
                      {slot.name}
                    </h5>
                  ...
                  <div className="space-y-2">
                    {slot.facets.map((facet) => (
                      ...
                      >
                        ...
                        <span className="wrap-break-word text-slate-600 sm:text-right">
                          {facet.value}
                        </span>
                      </div>
                    ))}
                    ...
                    <div className="mt-4 border-t border-slate-200 pt-4">
                      ...
                      <div className="space-y-2">
                        {slot.demons.map((demon) => (
                          ...
                          >
                            <p className="text-sm font-medium wrap-break-word text-amber-900 sm:text-base">
                              {demon.type}
                            </p>

                            <p className="mt-1 text-xs leading-5 wrap-break-word text-amber-800 sm:text-sm">
                              {demon.description}
                            </p>
                          </div>
                        ))}
            ...
            <div className="space-y-2">
              {relatedRelations.map((relation) => (
                ...
                >
                  ...
                  <p className="mt-1 text-sm wrap-break-word text-slate-500">
